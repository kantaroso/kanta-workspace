...
build-vue-base:
	$(eval TAGDATE := $(shell date "+%Y%m%d%H%M%S"))
	docker build -f ./infra/docker/vue/base/Dockerfile -t kantaroso/kanta-workspace-vue-base:latest .
	docker push kantaroso/kanta-workspace-vue-base:latest
build-php-prod:
	$(eval TAGDATE := $(shell date "+%Y%m%d%H%M%S"))
...
build-vue-prod:
	$(eval TAGDATE := $(shell date "+%Y%m%d%H%M%S"))
	docker build -f ./infra/docker/vue/laravel/Dockerfile -t kantaroso/kanta-workspace-vue-prod:${TAGDATE} -t kantaroso/kanta-workspace-vue-prod:latest .
	docker push kantaroso/kanta-workspace-vue-prod:${TAGDATE}
	docker push kantaroso/kanta-workspace-vue-prod:latest
build-go-pord:
	$(eval TAGDATE := $(shell date "+%Y%m%d%H%M%S"))
